...
class ReportController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        if ($request->resource == 'sales') {
            $sales = Sales::whereBetween(DB::raw('DATE(created_at)'), [$from_date, $to_date])->get();
        ...
        if ($request->resource == 'products') {
            ...
            $products = Product::whereBetween(DB::raw('DATE(created_at)'), [$from_date, $to_date])->get();

            return view('reports', compact('title', 'products'));
        }
        if ($request->resource == 'purchases') {
            $title = 'Purchases Reports';
            $purchases = Purchase::whereBetween(DB::raw('DATE(created_at)'), [$from_date, $to_date])->get();
